...
export const nrfdevice = (
	...
	const jobsManager = new NrfJobsManager(device, log, jobExectutionPath);
		...
		if (!onConnect || onConnectExecuted) {
			...
			await apiConn
				...
				.catch((err) => {
					...
					if (code !== 40410) {
						log.error(
							`Error getting data for device "${deviceId}". Cannot initialize jobs listener. Error: "${err?.response?.data
